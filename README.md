...
可以定制两类内容：手机端的界面与交互（主题、布局、按钮），以及手机端能调用的电脑能力（查看电脑文件、运行电脑上的程序）。`/mobile` 把需求交给 DSH agent，由 agent 直接修改本机 DSH 配置目录（`$DSH_HOME/mobile-access/`）下的文件，手机端持续检查并几秒内应用；界面相关是 `mobile.css`/`mobile.js`，需要电脑能力时则是 `extensions/` 下的扩展，其 `host.mjs` 以本机用户权限在电脑端运行。不修改 DeepSeek Harness 源码。
