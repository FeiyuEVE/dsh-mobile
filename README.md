...
- 在 DSH 对话中直接修改手机端布局、交互和功能，打开的手机页面约一秒内刷新——用对话定制自己的移动端，而不是写代码。
- 专属移动布局：会话抽屉、工具详情、设置和输入栏都按触屏重新组织。
- 自动发现局域网内的 DSH；切换 Wi-Fi、热点或 IP 后通常自动恢复，无需重新配对。
- 扫码、配对链接或密钥均可配对，不必手动输入 43 位密钥。
...
保存后，已打开的 App 和浏览器通常会在约一秒内应用变化。自定义不限于配色：`mobile.js` 可以添加导航、快捷操作、状态面板、相机、语音、扫码，以及调用同源 DSH API 的完整交互。
