...
import { ensureManagedCa, materializeManagedSetup, parseManagedSetup, preferredLanInterfaceNames, selectLanNetwork, } from '../src/managed-setup.js';
function interfaceTable(address) {
    return {
        WLAN: [{
                address,
...
function interfaceEntry(address, prefix = 24) {
    return {
        address,
        netmask: '255.255.255.0',
        family: 'IPv4',
...
describe('managed DHCP setup', () => {
    ...
    it('keeps an explicit address and reports genuinely ambiguous physical LANs', () => {
        ...
        expect(() => selectLanNetwork(undefined, undefined, table)).toThrow('Ethernet=192.168.10.8, WLAN=192.168.50.23');
    });
...
//# sourceMappingURL=managed-setup.test.js.map