...
import { apply, bindClientResponseLifetime, combineClientSignalLifetime, combineClientSignals, diagnosticEntriesForRender, diagnosticOverallForChecks, diagnosticServerCopy, DIAGNOSTIC_REASON_MESSAGES, extensionAssetUrl, extensionGenerationHeaders, extensionRouteUrl, failClosedExtensionGenerationReplacement, handleMissingExtensionManifest, installDshLanguageBoundSurface, MOBILE_CONTROL_MESSAGES, normalizeDiagnosticOverall, normalizeDiagnosticStatus, parseMobileExtensionManifest, PerIdActivationLifecycle, publishAuthoritativeExtensionIds, reconcileRemovedExtensions, registerUniqueDisposable, renderDiagnosticPayloadSafely, selectMobileControlLocale, startExtensionChangeStream, startLifecycleRefreshScheduler, validateDiagnosticChecks, } from '../src/client.js';
...
describe('mobile-control localization', () => {
    ...
    it('keeps Italian, English, and Chinese catalogs in parity including v0.3 diagnostics', () => {
        ...
        expect(MOBILE_CONTROL_MESSAGES.en.diagnosticsCopied?.toLowerCase()).toContain('redacted report');
...
describe('custom asset refresh lifecycle', () => {
    ...
    it('times out a hung cycle, aborts its work, and keeps the scheduler moving', async () => {
        ...
        const fakeWindow = new FakeWindow();
        const signals = [];
        const refresh = vi.fn((signal) => {
            signals.push(signal);
            return signals.length === 1 ? new Promise(() => { }) : Promise.resolve();
        });
    ...
    it('keeps one event stream and reconnects with bounded exponential backoff', async () => {
        ...
        const stop = startExtensionChangeStream(changed, {
            window: fakeWindow,
            create: () => { const source = new FakeEventSource(); sources.push(source); return source; },
        });
...
describe('extension request isolation', () => {
    ...
    it('retains request abort wiring until a streamed response finishes', async () => {
        ...
        let source;
        const response = bindClientResponseLifetime(new Response(new ReadableStream({
...
describe('per-extension activation lifecycle', () => {
    it('detaches cancelled pending work, starts a replacement, and only late-disposes the orphan', async () => {
        const lifecycle = new PerIdActivationLifecycle();
        const disposed = [];
        ...
        const late = lifecycle.activate('demo', {}, cycle.signal, controller => {
            ...
            return {
                result: new Promise(resolve => { resolveLate = resolve; }),
...
describe('client extension manifest reconciliation', () => {
    ...
    it('updates authoritative ids and clears managed resources on manifest removal', async () => {
        ...
        const managedStyles = new Set();
        const removed = [];
        const disposeManaged = (id) => { removed.push(id); lifecycle.remove(id); managedStyles.delete(id); };
        ...
        await handleMissingExtensionManifest(() => { publishAuthoritativeExtensionIds(authoritative, new Set(), [managedStyles], disposeManaged); }, async () => [false], new AbortController().signal);
...
//# sourceMappingURL=client-lifecycle.test.js.map