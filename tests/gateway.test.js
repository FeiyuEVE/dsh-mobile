...
async function udpDiscovery(port) {
...
async function trustedHttpsRequest(port, path, rootCert) {
    return new Promise((resolve, reject) => {
...
async function tlsFixtureFiles() {
...
function cookiesByName(headers) {
    ...
    for (const line of headers['set-cookie'] ?? []) {
        ...
        if (equals > 0)
            result.set(pair.slice(0, equals), pair.slice(equals + 1));
    }
    return result;
}
...
async function upstream(boot = 'legacy', requireAuthentication = false) {
    const observations = [];
    ...
    return {
        ...
        observations,
        upgradeObservations,
        releaseHold: () => { for (const release of held.splice(0))
            release(); },
    };
}
async function gateway(upstreamPort, overrides = {}, testSessionTtlMs, upstreamAuthenticatedUrl) {
...
async function openWebSocket(instance, path, session, origin, fetchSite = 'same-origin') {
...
describe('HTTP gateway', () => {
    ...
    it('keeps pairing and device management on a loopback Host-fenced route', async () => {
        ...
        const pairing = JSON.parse(opened.body);
    ...
    it('discovers one instance and renews a native long-lived device credential', async () => {
        ...
        const credential = JSON.parse(paired.body);
    ...
    it('uses mobile-sized pages and compresses session history', async () => {
        ...
        const historyRequest = (maxMessages) => JSON.stringify({
            type: 'client-request',
    ...
    it('renews, logs out, and revokes without exposing the persistent credential to the app path', async () => {
        ...
        expect(renewed.status).toBe(200);
        const renewedBody = JSON.parse(renewed.body);
    ...
    it.each([
        ['an appended intermediate file', (files) => ({
                mode: 'provided',
                certFile: files.leaf,
                keyFile: files.key,
                caFile: files.intermediate,
            })],
        ['a fullchain certificate file', (files) => ({
                mode: 'provided',
...
describe('WebSocket gateway', () => {
    ...
    it('allows only the known DSH event and Remote paths, forwards only the Session Cookie, and closes all on revocation', async () => {
        ...
        const secondClosed = new Promise(resolve => { second.socket.once('close', () => resolve()); });
        const thirdClosed = new Promise(resolve => { third.socket.once('close', () => resolve()); });
