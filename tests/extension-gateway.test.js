...
function pendingRequest(port, path, method, headers) {
    let settle;
    let fail;
    const received = new Promise((resolve, reject) => { settle = resolve; fail = reject; });
    ...
    const result = received.then(response => new Promise((resolve) => {
        const chunks = [];
        response.on('data', chunk => chunks.push(Buffer.from(chunk)));
...
async function openEventStream(port, headers) {
    ...
    const waitFor = (value) => new Promise((resolve, reject) => {
        if (body.includes(value)) {
            resolve(body);
            return;
        }
        const timeout = setTimeout(() => { cleanup(); reject(new Error(`event stream did not contain ${value}`)); }, 4_000);
        const data = () => { if (body.includes(value)) {
            cleanup();
            resolve(body);
        } };
        const ended = () => { cleanup(); reject(new Error('event stream ended early')); };
        const cleanup = () => {
...
describe('gateway extension namespace', () => {
    it('authenticates actions and routes while keeping the upstream proxy intact', async () => {
        ...
        cleanups.push(async () => { upstream.closeAllConnections(); await new Promise(resolve => upstream.close(() => resolve())); });
        ...
        const csrf = JSON.parse(paired.body);
    ...
    it('routes old mobile UI requests to their retained Host and asset generation', async () => {
        ...
        cleanups.push(() => context.fiber.dispose());
        const service = new MobileAccessService(context);
        await service.startLocal(extensionRoot, context);
    ...
    it('pushes credential-free extension changes and closes the stream when its device is revoked', async () => {
        ...
        await expect(Promise.race([
            disconnected,
            new Promise((_, reject) => setTimeout(() => reject(new Error('event stream remained open after revocation')), 2_000)),
...
//# sourceMappingURL=extension-gateway.test.js.map