...
async function createExtension(root, host, script = 'window.dshMobile.define({ apiVersion: 1, id: "demo", activate() {} })') {
...
describe('mobile extension registry', () => {
    it('validates manifests and rejects duplicate ids', () => {
        ...
        const definition = { schemaVersion: 1, id: 'sample', name: 'Sample', version: '1.0.0', actions: { ping: { run: async () => ({ ok: true }) } } };
    ...
    it('loads local host actions and swaps generations without exposing traversal', async () => {
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extensions-'));
    ...
    it('keeps the immediately previous Host generation while the browser stages new UI', async () => {
        ...
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-generations-'));
        directories.push(root);
    ...
    it('removes a deleted extension generation immediately', async () => {
        ...
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-delete-'));
        directories.push(root);
      ...
      api.action('ping', { async run() { return true } })
        ...
        delete globals.__removedCleanup;
        delete globals.__removedSignal;
    });
    it('keeps a route request lifetime until its response stream settles', async () => {
        const globals = globalThis;
        globals.__routeAborted = 0;
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-stream-'));
        directories.push(root);
      ...
      export default async api => api.route({ method: 'GET', path: '/events', handle(request) {
        ...
        cancelledResponse.body.destroy();
        ...
        const responseBody = completedResponse.body;
    ...
    it('publishes refreshes before bounded teardown and remains refreshable when a disposer never settles', async () => {
        ...
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-disposer-'));
        directories.push(root);
    ...
    it('cancels an in-flight staging Host before stopLocal returns', async () => {
        ...
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-stop-refresh-'));
        directories.push(root);
      ...
      try { api.effect(() => { globalThis.__stopLateEffectSetup += 1 }) }
        ...
        expect(service.manifest()).toEqual([]);
        expect(service.status()).toEqual({ loaded: 0, failed: 0 });
        delete globals.__stopActivationGate;
        delete globals.__resolveStopActivation;
        delete globals.__stopActivationStarted;
        delete globals.__stopEffectSetup;
        delete globals.__stopEffectCleanup;
        delete globals.__stopLateEffectSetup;
        delete globals.__stopLateRejected;
    });
    it('does not revive the watcher when stopLocal overlaps the initial refresh', async () => {
        ...
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-start-stop-'));
        directories.push(root);
      ...
      try { api.effect(() => { globalThis.__startStopLateSetup += 1 }) }
        ...
        expect(service.manifest()).toEqual([]);
        expect(service.status()).toEqual({ loaded: 0, failed: 0 });
        delete globals.__startStopGate;
        delete globals.__resolveStartStop;
        delete globals.__startStopStarted;
        delete globals.__startStopSetup;
        delete globals.__startStopCleanup;
        delete globals.__startStopLateSetup;
        delete globals.__startStopLateRejected;
    });
    it('snapshots assets into the generation and refreshes on asset-only changes', async () => {
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-assets-'));
        directories.push(root);
    ...
    it('rejects an assets symlink or junction that leaves the extension directory', async () => {
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-asset-link-'));
        directories.push(root);
        const outside = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-outside-'));
    ...
    it('rejects registrations after activation timeout and cleans an effect that resolves late', async () => {
        const globals = globalThis;
        globals.__lateSetup = 0;
        globals.__lateCleanup = 0;
        globals.__lateRejected = 0;
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-timeout-'));
        directories.push(root);
        await createExtension(root, `export default async api => {
      ...
      try { api.effect(() => { globalThis.__lateSetup += 100 }) } catch { globalThis.__lateRejected += 1 }
        ...
        delete globals.__lateSetup;
        delete globals.__lateCleanup;
        delete globals.__lateRejected;
    }, 12_000);
    it('keeps the complete previous client generation when a replacement host fails', async () => {
        const root = await mkdtemp(join(tmpdir(), 'dsh-mobile-extension-rollback-'));
        directories.push(root);
        const directory = join(root, 'demo');
        await (await import('node:fs/promises')).mkdir(directory, { recursive: true });
        await writeFile(join(directory, 'extension.json'), JSON.stringify({ schemaVersion: 1, id: 'demo', name: 'Demo', version: '1.0.0' }));
