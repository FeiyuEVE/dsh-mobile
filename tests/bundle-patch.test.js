...
function mobileRow() {
    const patches = yaml.load(source, { schema });
    const rows = patches.flatMap(patch => record(patch).insert ?? [patch]).map(record);
    const matches = rows.filter(row => row.id === 'mobile-access');
    if (matches.length !== 1)
        throw new Error('expected one mobile-access row');
    return matches[0];
}
function rows() {
    const patches = yaml.load(source, { schema });
    return patches.flatMap(patch => record(patch).insert ?? [patch]).map(record);
}
function evaluate(value) {
    const expression = record(value).__jsExpr;
    if (typeof expression !== 'string')
        throw new TypeError('expected JavaScript scalar');
    const run = Function('dshHomePath', `"use strict"; return (${expression});`);
...
//# sourceMappingURL=bundle-patch.test.js.map